...
app.get('/verificar/:user/:token', async(req, res)=>{
    if(db().verificacion_correcta(req.params.user, req.params.token)){
        db().actualizar(req.params.user, 'email_verificado', 'true')
        res.redirect('/sign_up/autentication/index.html?true')
    }else{
        res.redirect('/sign_up/autentication/index.html?false')
    }
})
...
app.post('/pantalla', async(req, res)=>{
